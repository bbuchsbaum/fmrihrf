...
sframe <- sampling_frame(
  blocklens = c(120, 120),  # Two 2-minute blocks
  TR = 2                    # 2-second TR
)
print(sframe)
...
design_plot_df$Block <- rep(1:2, each = 60) # 60 scans per block
...
ggplot(design_plot_long, aes(x = Time, y = Response, color = Condition)) +
  geom_line(linewidth = 1) +
  geom_vline(xintercept = 120, linetype = "dashed", alpha = 0.7) +
  scale_color_viridis_d() +
...
ggplot(complex_long, aes(x = Time, y = Response, color = Regressor)) +
  ...
  geom_vline(xintercept = c(180, 360), linetype = "dashed", alpha = 0.5) +
  scale_color_viridis_d() +
