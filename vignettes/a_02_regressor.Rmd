...
`dt` and convolves that signal with the HRF. By default the series is demeaned
before convolution so the mean does not create run-edge transients; native
...
If the feature is only meaningful inside stimulus blocks, pass a `mask` for
the on-period and keep a separate boxcar for "stimulus on" rather than
demeaning the whole concatenated series.
