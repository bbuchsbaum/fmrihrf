...
An object of class \`HRF\` used for convolution, OR a list of HRF
objects (one per event) for trial-varying HRFs. \* \`duration\`: Numeric
vector of event durations (seconds). \* \`amplitude\`: Numeric vector of
event amplitudes/scaling factors. \* \`span\`: Numeric scalar indicating
